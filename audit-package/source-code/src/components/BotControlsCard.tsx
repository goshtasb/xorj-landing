...
export function BotControlsCard() {
  ...
  const ensureAuthentication = async () => {
    ...
    try {
      console.log('🔐 Authenticating with gateway...', effectivePublicKey);
  ...
  // Fetch bot status using the new authenticated bot service
  const fetchBotStatus = async () => {
    ...
    try {
      ...
      console.log('🔄 Fetching bot status for authenticated user');
      
      // Use the new authenticated bot service (no user_id needed)
  ...
  const toggleBot = async () => {
    ...
    // If enabling bot, validate investment amount against wallet balance
    if (!isCurrentlyActive && userSettings?.investmentAmount) {
      const validation = await walletBalanceService.validateInvestmentAmount(
        effectivePublicKey!, 
        userSettings.investmentAmount
      );
      
      if (!validation.isValid) {
        setError(`Cannot enable bot: ${validation.error}`);
        return;
      }
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        // Update bot status immediately in the UI
        setBotStatus(prevStatus => ({
          ...prevStatus!,
          status: isCurrentlyActive ? 'stopped' : 'active',
          configuration: {
            ...prevStatus?.configuration,
            enabled: !isCurrentlyActive
          }
        }));
        
        // Show success feedback and refresh after a delay to let user see the change
        console.log('🔄 Bot state updated in UI, refreshing in 2 seconds...');
        setTimeout(() => {
  ...
  const saveInvestmentAmount = async () => {
    ...
    const validation = await walletBalanceService.validateInvestmentAmount(effectivePublicKey!, amount);
    if (!validation.isValid) {
  ...
  const emergencyStop = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Update bot status immediately to stopped
        setBotStatus(prevStatus => ({
          ...prevStatus!,
          status: 'stopped',
          configuration: {
        ...
        // Show success feedback and refresh after a delay
        console.log('🚨 Emergency stop executed, refreshing in 2 seconds...');
        setTimeout(() => {
          window.location.reload();
        }, 2000);
  ...
  // Auto-refresh bot status, settings, and wallet balance when wallet is connected and authenticated
  useEffect(() => {
    if (!isWalletReady) return;
    ...
    // Only set up interval if authentication is working
    const interval = setInterval(() => {
      if (authenticated && isAuthenticated()) {
        fetchBotStatus();
        fetchUserSettings();
        fetchWalletBalance(); // Refresh balance periodically
      }
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-black/20 rounded-lg p-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={toggleBot}
                  disabled={loading || authenticationLoading || toggleLoading || botStatus.status === 'error'}
                  className={`
                  ...
                  {loadingBalance ? (
                    ...
                    <>
                      ...
                      <div className="text-xs text-gray-400">
                        Max investable: {walletBalanceService.formatBalance(Math.max(0, walletBalance * 0.98 - 10))}
                      </div>
                      {walletBalance === 0 && (
                ...
                {parseFloat(investmentAmount) > 0 && parseFloat(investmentAmount) > (walletBalance * 0.98 - 10) && (
                  <div className="mt-1 p-2 bg-red-500/10 border border-red-500/20 rounded text-xs text-red-400 flex items-center gap-1">
                    ...
                    {walletBalance === 0 
                      ...
                      : `Amount exceeds available balance of ${walletBalanceService.formatBalance(Math.max(0, walletBalance * 0.98 - 10))}`
                    }
                  </div>
                )}
              </div>
