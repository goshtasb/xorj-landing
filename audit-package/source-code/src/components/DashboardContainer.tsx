...
export function DashboardContainer({ className = '' }: DashboardContainerProps) {
  ...
  // Only use real wallet address when connected, fallback for development
  const effectivePublicKey = mounted ? (
    publicKey?.toString() || 
    (process.env.NODE_ENV === 'development' ? '5QfzCCipXjebAfHpMhCJAoxUJL2TyqM5p8tCFLjsPbmh' : undefined)
  ) : undefined;
  ...
  // Show wallet required only in production mode without wallet
  if (!effectivePublicKey && process.env.NODE_ENV === 'production') {
    return (
      <div className={`space-y-6 ${className}`}>
        <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl p-6">
          <div className="text-center py-8">
