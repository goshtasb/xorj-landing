...
export class KeyManagementService {
  ...
  async getEncryptionKey(keyId: string = 'default'): Promise<Buffer> {
    ...
    try {
      ...
      console.error(`Failed to retrieve encryption key ${keyId}:`, error);
   ...
   */
  private async fetchKeyFromKMS(keyId: string): Promise<Buffer> {
   ...
   */
  private async encryptWithKMS(key: Buffer, keyId: string): Promise<string> {
