...
if (process.env.NODE_ENV === 'production' || process.env.DATABASE_HOST || process.env.DATABASE_URL) {
  ...
  QueryResult = class MockQueryResult {};
}
...
export function initializeDatabase(): any {
  ...
  try {
    ...
    throw new CriticalDatabaseError(
      `Cannot initialize database connection: ${error instanceof Error ? error.message : 'Unknown error'}`,
      'CONNECTION_FAILED'
    );
  }
}
...
export type DatabaseResult<T = any> = QueryResult<T>;
