...
pub mod vault {
    ...
    pub fn withdraw(ctx: Context<Withdraw>, amount: u64) -> Result<()> {
        ...
        // Get vault data for PDA seeds (before any borrowing)
        ...
        // Update vault state (now we can borrow mutably)
        let vault = &mut ctx.accounts.vault;
    ...
    pub fn bot_trade(
        ...
        let _balance_before = ctx.accounts.vault_token_account.amount;
        let _output_balance_before = ctx.accounts.vault_output_token_account.amount;
