...
class BotServiceClient {
  ...
  private loadSessionToken(): void {
    if (typeof window !== 'undefined') {
      ...
      this.sessionToken = localStorage.getItem('xorj_session_token') || localStorage.getItem('xorj_jwt_token');
    }
  }
  ...
  /**
   * Reload session token from localStorage (public method)
   */
  public reloadSessionToken(): void {
    this.loadSessionToken();
  }
   ...
   */
  async authenticate(walletAddress: string, signature: string, message: string = 'XORJ Authentication'): Promise<AuthResponse> {
