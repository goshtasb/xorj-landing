...
function getConnection(): Pool {
  if (!fastPool) {
    ...
    fastPool = new Pool({
      ...
      min: 2,  // Reduced minimum number of connections
...
export async function fastQuery<T = unknown>(
  query: string, 
  params: unknown[] = [],
  ...
  if (useCache && query.trim().toLowerCase().startsWith('select')) {
    ...
    if (cached) {
      console.log(`⚡ Cache hit: ${Date.now() - _startTime}ms`);
      return cached;
    }
  }
  ...
  try {
    ...
    const duration = Date.now() - _startTime;
    console.error(`❌ Fast query error: ${duration}ms -`, queryError);
    throw queryError;
  } finally {
    if (client) {
      client.release();
...
export async function fastTransaction<T>(callback: (query: typeof fastQuery) => Promise<T>): Promise<T> {
  ...
  try {
    ...
    const txQuery = async <U = unknown>(query: string, params: unknown[] = []): Promise<U[]> => {
      const result = await client.query(query, params);
      return result.rows;
    };
