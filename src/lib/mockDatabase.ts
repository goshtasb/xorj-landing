...
import { 
  ServiceResponse,
  UserSettings,
  // CreateUserSettingsData, // Unused
  UpdateUserSettingsData,
  BotState,
  // CreateBotStateData, // Unused
  UpdateBotStateData,
  Trade,
  CreateTradeData,
  TradeFilters
    ...
    async create(data: CreateTradeData): Promise<ServiceResponse<Trade>> {
      try {
        ...
        if (existingTrade) {
          ...
          (mockError as MockDatabaseError).code = '23505';
          (mockError as MockDatabaseError).constraint = 'trade_idempotency_key';
          throw mockError;
        }
